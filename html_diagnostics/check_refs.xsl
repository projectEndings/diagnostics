...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="baseUris" select="for $n in $uniqueRefs return hcmc:getBaseUri($n)"/>
        ...
        <xsl:if test="not(empty($docErrors))">
                <xsl:message>Found <xsl:value-of select="$docErrorsCount"/> errors</xsl:message>
               ...
               <xsl:sequence select="$docErrorsOut"/>
                <xsl:for-each-group select="$hashedRefs" group-by="hcmc:getBaseUri(.)">
                    ...
                    <xsl:if test="empty($docErrors[.=$baseUri])">
